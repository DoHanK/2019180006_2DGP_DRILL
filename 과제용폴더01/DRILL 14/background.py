...
class TileBackground:
    ...
    def draw(self):
        ...
        title_left = (self.window_left//800)%3
        title_right = ((self.window_left+self.canvas_width)//800)%3
        left_offset = self.window_left%800

        title_bottom = (self.window_bottom//600)%3
        title_top = ((self.window_bottom+self.canvas_height)//600)%3
        bottom_offset= self.window_bottom%600

        for ty  in range(title_bottom,title_top+1):
            for tx in range(title_left,title_right+1) :
                self.tiles[ty][tx].draw_to_origin(-left_offset+(tx-title_left)*800, - bottom_offset+(ty-title_bottom)*600)
