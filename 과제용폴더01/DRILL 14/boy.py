...
class WalkingState:
    ...
    def do(boy):
        ...
        boy.x=clamp(25,      boy.x       ,server.background.w-1)
        boy.y=clamp(25,     boy.y       ,   server.background.h-1)

    def draw(boy):
        sx, sy = boy.x-server.background.window_left, boy.y-server.background.window_bottom
        draw_rectangle(*boy.get_bb())
        boy.font.draw(sx - 40, sy + 40, '(%d, %d)' % (boy.x, boy.y), (255, 255, 0))
        boy.font.draw(sx - 40, sy + 60, '(score %d)' % (boy.score), (255, 255, 0))
...
class Boy:
    ...
    def get_bb(self):
        sx, sy = server.background.window_left,  server.background.window_bottom
        return self.x-sx - 50, self.y-sy - 50, self.x-sx + 50, self.y-sy + 50
